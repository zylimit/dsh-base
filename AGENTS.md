...
8. **A review must end.** `catalog.review.maxRounds` (default 3) caps consecutive `FIX_REQUIRED` verdicts on one change; at the limit the verdict reports `escalate` and says stop — either the change is wrong or the standard is, and no further round can tell you which. Take it to a human.
