...
1. Name the command that would prove it. (prompt-only)
2. Run it fresh. Never reuse an earlier run across an edit. (prompt-only)
3. Read the full output **and** the exit code. (prompt-only)
4. Confirm the output actually supports the specific claim. (prompt-only)
5. Only then say it. (prompt-only)
...
1. Work from a task envelope: **Goal · Scope · Out of scope · Existing pattern · Verification · Escalation**. A task without all six is not ready; ask, do not guess. (prompt-only)
2. Change only files inside Scope. A necessary change outside Scope is a new task, not an extension of this one. (prompt-only)
3. No drive-by refactors, no opportunistic renames, no reformatting files you did not otherwise change. (prompt-only)
4. Reuse the pattern named in the envelope before inventing a new one. (prompt-only)
...
6. Missing information is not permission. (prompt-only)

## 4. Architecture law — preventing decay

1. `.dsh/base/catalog.json` is the authoritative architecture. A design that cannot be expressed as modules, layers and forbidden edges is not finished. Enforced by `arch-check`.
...
1. `critical` and `high` **block**: the gate does not pass unless a check that claims the attribute actually passed for the affected module. Enforced by `gate`.
2. Counter-evidence outranks confirming evidence. One claiming check that FAILs or is BLOCKED reopens the gap even if others passed. Enforced by `gate`.
3. `SKIPPED` covers nothing. Declared but unwired is a visible gap, not a pass. Enforced by `gate` and `attributes`.
4. `minimal` and `none` require a written `attributeReasons` entry. Opting out of governance is a recorded decision, never a free default. Enforced by `catalog-lint`.
5. **`security`, `safety` and `privacy` are protected**: never waivable, never fast-skippable, never downgraded by a waiver. There is no expressible waiver for them. Enforced by `waiver` and `fast`.
...
2. **It cannot touch the protected floor.** Every check claiming `security`, `safety` or `privacy` runs regardless. Those are not slow; they are why the software is allowed to exist. Enforced by `gate`.
3. **It skips only what was marked `allowFastSkip` in advance**, while there was time to think about which evidence is cheap to defer. Deciding that during the emergency is how everything becomes skippable. Enforced by `gate`.
...
4. **One lens finding an error is not outvoted.** Rule 5.2 applies here unchanged: counter-evidence outranks confirming evidence, so four clean lenses never cancel one located error. Enforced by `review`.
...
7. **Reviews advance in stages, and a stage is the budget.** Stage 1 `code` (correctness, architecture, maintainability) must fully report before stage 2 `functional` (testing, performance), which must pass before stage 3 `trust` (security, privacy, reliability, resilience). `review lens` refuses an early report for a later stage (exit 1, `stageGated`), so expensive lenses are never spent on code that has not passed the cheap ones. The profile sets the team — `personal` convenes correctness only, `team` adds testing and architecture, `production` adds security, reliability and performance, `regulated` convenes all nine (including maintainability) — and a lens whose attribute no affected module declares above `minimal` is not convened. The four hygiene lenses sit in the deeper profiles: they improve the code, they do not decide whether it may ship. Attributes can only shrink the team, never grow it; risk and operations can only raise it. An affected module at `high` risk convenes at least the `team` profile, at `critical` risk at least `production`, and the release gate refuses a receipt that convened fewer lenses than the configured release floor (default `production`, never below `team`).
8. **A review must end.** `catalog.review.maxRounds` (default 3) caps consecutive `FIX_REQUIRED` verdicts on one change; at the limit the verdict reports `escalate` and says stop — either the change is wrong or the standard is, and no further round can tell you which. Take it to a human. Enforced by `review`.
9. **A finding may be carried, never deleted.** `review backlog add` records a finding a human decides to carry, with owner and expiry; `review backlog list` reports expirations. A security, safety or privacy finding is never backloggable — the backlog would be the waiver the design refuses.

## 6. Verification law

1. Verification is impact-scoped, never "run everything" and never "run what feels related". `impact` decides.
2. An unmapped path, a global path, a truncated file list or a non-git tree forces a conservative full fan-out marked `degraded`. Over-testing is cheap; a missed regression is not. Enforced by `impact`.
...
4. The ledger is hash-chained. A broken chain fails closed: every prior verification is treated as unproven until re-run. Enforced by `ledger`.
5. A waiver pre-declares a skip before a check runs; once a check has executed, its FAIL or BLOCKED is an immutable ledger fact that no waiver rewrites. Never edit a receipt or waiver by hand. Enforced by `waiver` and `ledger`.
6. Never re-run a check with different arguments to obtain a greener answer, and never delete evidence. (prompt-only)
...
2. Never read a secret file to "check" it, and never pipe one into another command. Read the `.example` instead. (prompt-only)
3. Deny by default at every boundary. Every outbound call has a timeout. Every retry has a bound, a backoff and jitter. Every queue and cache has a limit. (prompt-only)
4. Failures fail safe, not open. A component that cannot establish its invariants refuses to serve, loudly. (prompt-only)
5. Errors are handled or propagated with context. Never swallowed (`no-silent-failure`).

## 8. Memory law

1. `progress.md` is project memory and is **committed**. It is project state, so another machine resumes from it; the tool under `.dsh/` may be private, the memory never is. (prompt-only)
...
3. A `Done` entry without an evidence pointer is not written. A `Decisions` entry without the rejected alternative is a status update and belongs under `Done`. Hedged language is demoted to `Notes` tagged `Needs-Confirmation`. (prompt-only)
...
1. One repository per service, each small enough that one agent holds its whole model, is the right answer to context. It moves complexity from file dependencies to **contracts between repositories**, and nothing inside a repository can see that surface. (prompt-only)
...
3. **Never change a published contract version in place.** Publish the new version beside the old, declare a `sunset` date, migrate every consumer, then retire. A deprecation with no sunset date is an error, not a label. Enforced by `fleet`.
...
5. A contract cycle between repositories means they cannot be released independently. That is the distributed-monolith signature: report it, do not normalise it. Enforced by `fleet`.
...
1. Delegate evidence-gathering. Retain judgement. A delegate's claim is accepted only with an evidence pointer. (prompt-only)
...
3. Start independent delegations in one message. Never sleep-poll a background job. (prompt-only)
4. Serialize writers: at most one delegate writes to a given module at a time. (prompt-only)
5. A `BLOCKED` result escalates in this order: supply the missing context → raise the model → split the task → report the limitation. Never retry unchanged. (prompt-only)

## 10. Agent efficiency law

1. Code Mode: batch independent reads, searches and commands into one `run_code` program. One tool call per file is the main avoidable cost in this harness. (prompt-only)
2. `grep`/`glob` before `read`. Do not read a large file in full when a targeted search answers the question. Never read generated output, lockfiles or vendored trees. (prompt-only)
3. Write intermediate findings to a file and pass the path; do not paste large content into messages. (prompt-only)
4. Keep this file small. Detail belongs in a skill, which is loaded only when it is needed. (prompt-only)
