...
- Dev-process supervisor: `node .dsh/base/supervisor.mjs start|stop|status` guards a long-running development process with exponential backoff, an optional HTTP health probe that kills wedged-but-alive children, and a restart-storm circuit breaker that fails visibly; status reports pid liveness, never stale records.
- The golden ruler covers a real gate run (five repo states, 47 rows): command spawning, evidence capture, the ledger append and attribute coverage are byte-stable parts of the contract, with check commands chosen for cross-platform determinism.
- 94 engine self-test assertions and 152 behavioural tests; the scaffold governs itself.
