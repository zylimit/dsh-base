...
| 51 | Project settings file | cc-base, opencode-base | Adapted | `catalog.json` is the single project switch; runtime configuration lives in `$DSH_HOME/profiles/<name>`, with an optional repository `cordis.patch.yml` applied via `dsh --patch` and inspected with `dsh --dump-config` | dsh has no project settings file; splitting project policy (committed) from runtime configuration (per operator) keeps both honest |
