...
Review by trying to break the change, not by confirming it looks fine. Three gates in fixed order (static, spec compliance, quality), then a Blue/Red/Judge protocol that ends in `ACCEPT | FIX_REQUIRED | NEEDS_MORE_EVIDENCE` and, on ACCEPT, a diff-bound receipt.
...
- `node .dsh/base/dsb.mjs receipt verify` exits 4 (stale evidence: the tree moved after the last receipt).
...
1. Build the evidence pack first: `node .dsh/base/dsb.mjs review-pack`. Review that pack. Do not assemble your own diff view: a self-selected diff is where reviewers skip files. Exit 3 means governance is not configured; report that instead of pretending to review.
...
6. Run the Blue/Red/Judge protocol. Blue and Red are separate passes with separate outputs; Judge decides.

   | Role | Output |
   | --- | --- |
   | Blue | What was verified, how it was verified, with command output or `path:line` per claim. Unverified claims are listed as unverified. |
   | Red | Concrete attacks: each finding has `path:line` or a reproduction path, an impact, and a severity. Speculation without a location is discarded. |
   | Judge | Weighs Blue evidence against Red findings and returns exactly one verdict |

   Verdicts: `ACCEPT` (no unresolved finding at severity major or above), `FIX_REQUIRED` (a concrete finding with a location), `NEEDS_MORE_EVIDENCE` (the claim cannot be judged from the pack; name the exact evidence needed).
...
8. On `ACCEPT` write the receipt: `node .dsh/base/dsb.mjs receipt write` with `taskId`, `reviewer`, `verdict`, `scope`, and `notes` containing the Root cause line for a defect fix. The receipt binds to the current diff hash; any byte changed afterwards stales it (`receipt verify` exits 4) and the review must be redone against the new diff.
9. Close the task: `node .dsh/base/dsb.mjs task complete`. It refuses while there is no passing gate bound to this diff, no fresh ACCEPT receipt, a broken ledger, or an empty verification plan. Do not work around a refusal; fix what it names.
...
- `review-pack` or `gate` exits 3 (degraded/not configured): you cannot review evidence that was never produced.
...
| Finding written as "error handling could be better" | A finding needs `path:line` or a reproduction path, an impact and a required fix - otherwise it is not a finding. |
